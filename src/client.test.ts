...
describe('HeadlessCommerceClient', () => {
  ...
  it('looks up an order with shopper proof and never retries the private input', async () => {
    const result = {data:{orderNumber:'ORD1',status:'pending',items:[],tracking:null,fulfillment:{mode:'pickup',pickupLocationId:'loc-1',pickupStatus:'pending',pickupReadyAt:null,pickupLocation:{id:'loc-1',name:'Saved store'}}},requestId:'r'};
    const fetcher = vi.fn(async () => new Response(JSON.stringify(result),{status:201}));
    const client = new HeadlessCommerceClient({baseUrl:'https://sandbox.test',publishableKey:'pk_test_demo',fetch:fetcher as typeof fetch,maxRetries:2});
    const order = (await client.orders.lookup('ORD1','buyer@example.test')).data;
    expect(order.status).toBe('pending'); expect(order.fulfillment).toEqual(result.data.fulfillment);
    expect(fetcher).toHaveBeenCalledTimes(1);
  ...
  it('covers customer session and account route families', async () => {
    ...
    const client = new HeadlessCommerceClient({baseUrl:'https://sandbox.test',publishableKey:'pk_test_demo',fetch:fetcher as typeof fetch,maxRetries:2});
    await client.customer.login('buyer@example.test','password1','hc_cart'); await client.customer.requestPasswordRecovery('buyer@example.test'); await client.customer.resetPassword('buyer@example.test','b'.repeat(64),'new-password'); await client.customer.refresh('a'.repeat(64));
    ...
    expect(fetcher.mock.calls.map((call)=>[new URL(String(call[0])).pathname,call[1]?.method])).toEqual([
      ['/v1/headless/customer/auth/login','POST'], ['/v1/headless/customer/auth/password/recovery','POST'], ['/v1/headless/customer/auth/password/reset','POST'], ['/v1/headless/customer/auth/refresh','POST'],
    ...
    expect(fetcher.mock.calls[4][1]?.headers).toMatchObject({'x-customer-token':'customer-jwt'});
    expect(fetcher.mock.calls[7][1]?.headers).toMatchObject({'Idempotency-Key':'return-intent-1'});
    expect(fetcher.mock.calls[6][0].toString()).toContain('page=2&limit=10&status=pending');
  });
